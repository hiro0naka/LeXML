...
class LeXML(object):
    def __init__(self, train_file_name, test_file_name, rep_file_name, embedding_method="line", sparse=True, num_partition=2,
                 learning_rate=0.005, weight_decay=0.01, dropout_rate=0.7, batch_size=128, epoch=10):
        self.num_partition = num_partition
        self.sparse = sparse
        if self.sparse:
            self.train_x, self.train_y = rd.read_spmtx(train_file_name)
            self.test_x, self.test_y = rd.read_spmtx(test_file_name)
            # self.train_x, self.train_y = rd.get_spdata(train_file_name)
            # self.test_x, self.test_y = rd.get_spdata(test_file_name)
        else:
    ...
    def lexml(self):
        ...
        # --- Get embedding data ---
        embedding_x, w = self.model(self.dataX)
    ...
    def model(self, input_x):
        ...
        w1 = self.weight_variable([self.feature_dim, self.embedding_dim])
        b1 = self.bias_variable([self.embedding_dim])
        h2 = tf.matmul(h1, w1) + b1
        # h2 = tf.matmul(h1, w1, a_is_sparse=self.sparse) + b1
        h2_drop = tf.nn.dropout(h2, self.keep_prob)
        self.output_x = tf.nn.l2_normalize(h2_drop, 1)
        return self.output_x, w1
    ...
    def setup_knn(self, embedding_train_x, n_neighbors):
        knn = KNeighborsClassifier(n_neighbors=n_neighbors, n_jobs=20)
        knn.fit(embedding_train_x, self.train_y)
        return knn

    def setup_cls_knn(self, embedding_train_x, n_neighbors):
        start_time = time.time()
    ...
    def predict_by_bruteforce(self, knn, embedding_test_x, n_neighbors, k_pred=5):
    ...
    def predict_by_clustering(self, Z, embedding_test_x, n_neighbors, k_pred=5):
        predict = np.zeros((self.num_test, 5))
    ...
    def eval(self, use_clustering=False, n_neighbors=30):
        print("-- predict -----")
        ...
        if use_clustering:
            knn = self.setup_cls_knn(embedding_train_x, n_neighbors)
            self.predict_by_clustering(knn, embedding_test_x, n_neighbors)
        else:
            Z = self.setup_knn(embedding_train_x, n_neighbors)
            self.predict_by_bruteforce(Z, embedding_test_x, n_neighbors)
...
if __name__ == "__main__":
    model = LeXML("dataset/Wiki10/wiki10_train.txt",
                  ...
                  "m_data/Wiki10/wiki10_li50_k20_1.embeddings",
